	...
	def __init__(self, port: int, verbose: bool = False):
		...
		Args:
			port: TCP port used by the RL process to listen for the agent
				connection. Valid ports are restricted by the underlying transport
				layer to the range ``20000`` to ``49151``.
	...
	def stepSendActGetObs(self, action, timeout: float = 10.0):
		...
		Args:
			action: Action object to send to the agent. It must be serializable by
				Python ``pickle`` because the transport exchanges pickled payloads.
			timeout: Timeout in seconds for each communication operation involved
				in the request. Values less than or equal to ``0.0`` disable the
				timeout at the transport layer.
	...
	def stepExpFinished(self, timeout: float = 10.0):
		...
		Args:
			timeout: Reserved for API symmetry with the other high-level methods.
				The current implementation sends the finish indicator immediately and
				does not consume the timeout value.
...
class AgentSide:
	...
	class WhatToDo(Enum):
		...
		Attributes:
			REC_ACTION_SEND_OBS: Receive a new action, report the duration of the
				previous action, and later send the observation that results from the
				new action.
	...
	def __init__(
		...
		Args:
			ipbaselinespart: IPv4 address of the RL process that owns the listening
				server socket. A common pattern is to pass ``BaseCommPoint.get_ip()``
	...
	def readWhatToDo(self, timeout: float = 10.0):
		...
		Args:
			timeout: Maximum number of seconds allowed for reading a pending
				command once the socket indicates that data are available. Values
				less than or equal to ``0.0`` disable the timeout at the transport
				layer.
	...
	def stepSendLastActDur(self, lat: float):
		...
		Args:
			lat: Actual duration, in seconds, of the action that was being executed
				before the newly received action replaced it. This value belongs to
				the agent clock domain.
	...
	def stepSendObs(self, obs, agenttime: float = 0.0, rew: float = 0.0):
		...
		Args:
			...
			rew: Reward associated with the current action, when reward
				computation is delegated to the agent side. If the RL process owns
				reward computation, leave this value at its default.
