...
class DecoupledLunarLanderEnv(gym.Env):
	...
	def _parse_payload(self, payload: Any) -> np.ndarray:
		...
		obs_payload = payload.get("observation", payload) if isinstance(payload, dict) else payload
		obs = np.asarray(obs_payload, dtype=np.float32)
		if obs.shape != self.observation_space.shape:
			raise ValueError(
				f"Invalid observation shape {obs.shape}; expected {self.observation_space.shape}"
			)
		return obs
	...
	def step(self, action):
		...
		if self._debug:
			print(
				"[RL] step action={} lat={:.6f}s ato={:.6f} wall={:.6f} rew_rl={:.6f}".format(
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument("--debug", action="store_true", help="Enable verbose RL-side logs")
...
def run_rollout(model: PPO, env: DecoupledLunarLanderEnv, rollout_steps: int) -> None:
	...
	if lats:
		...
		print(f"[RL] rollout finished. mean LAT over {len(lats)} steps: {mean_lat:.6f}s")
