...
def _normalize_obs(obs: np.ndarray | list[float] | tuple[float, ...] | dict[str, Any]) -> np.ndarray:
...
def compute_reward(
	obs: np.ndarray | list[float] | tuple[float, ...] | dict[str, Any],
	action: int | None,
	prev_obs: np.ndarray | list[float] | tuple[float, ...] | dict[str, Any] | None = None,
	lat: float | None = None,
) -> float:
...
def is_terminated(obs: np.ndarray | list[float] | tuple[float, ...] | dict[str, Any]) -> bool:
