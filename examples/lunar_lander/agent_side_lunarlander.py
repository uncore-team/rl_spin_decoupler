...
class LunarLanderAgent:
	...
	def _run_control_tick(self) -> None:
		...
		obs, reward, terminated, truncated, info = self._env.step(int(self._last_action))
			...
			while True:
				...
				if self._state == StepState.EXECUTING_LAST_ACTION:
					if now - self._last_action_start >= self._rl_step_period:
						...
						if self._debug:
							print(
								"[AGENT] sent transition obs action={} internal_done={}".format(
									self._last_action,
						...
						if self._reset_pending_after_publish:
							# Policy: after signaling env_done once, locally auto-reset
							# to keep simulator running and avoid stepping a done episode.
							self._reset_workspace()
					...
					self._comm.resetSendObs(self._build_transport_payload(), agenttime=now)
					if self._debug:
...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument("--debug", action="store_true", help="Enable verbose agent logs")
