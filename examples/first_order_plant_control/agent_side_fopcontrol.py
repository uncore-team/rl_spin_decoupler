...
class AgentVisualizer:
    ...
    def __init__(self, refresh_every: int = 5):
        ...
        self._plt = plt
        self._plt.ion()
        self._fig, (self._ax_state, self._ax_timing) = self._plt.subplots(
            2, 1, figsize=(10, 7)
        )
    ...
    def update(self, record: AgentVizRecord, step_state: StepState) -> None:
        ...
        self._ax_state.plot(
            ticks, target, label="target", linestyle="--", linewidth=1.8
        )
        ...
        self._ax_timing.plot(
            ticks, age_ms, label="time since last RL action (ms)", color="tab:orange"
        )
